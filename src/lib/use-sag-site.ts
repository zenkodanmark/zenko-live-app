...
function applyStatus<T extends { id: string; number: string; ledelseStatus?: LedelseStatus }>(rows: T[], kind: "tf" | "as" | "tb" | "er", db: DbState | null, hasLocal: boolean): T[] {
...
export function useSagSite(slug: string): { site: SagSite | null; missing: boolean; busy: boolean; reload: () => void } {
  ...
  const value = useMemo(() => {
    ...
    const allTb: Offer[] = applyStatus(offers as Offer[], "tb", db, hasLocal);
  ...
  }, [slug, projects, tfs, slips, offers, ents, fieldItems, db, busy, employeeId]);
