...
import { CrewSagHome } from "@/components/crew-sag-home";
...
function SvendInner() {
  ...
  useEffect(() => {
    if (search.e && search.p) {
      ...
      if (ok) {
        void nav({ to: "/svend", search: { e: "", p: "" }, replace: true });
        return;
      }
    }
    if (!ready) return;
    ...
    if (who) {
      login(who.id);
      if (pathForRole(who.role) === "/mester") void nav({ to: "/mester", search: { e: "", p: "" } });
      return;
    }
    void nav({ to: "/" });
...
function ksPhotoPool(days: Record<string, { photos: KsPhoto[] }>, drivePhotos: KsPhoto[]) {
  return [...softrKsPhotos(), ...Object.values(days).flatMap((d) => d.photos), ...drivePhotos];
}
...
function CrewKsView({ id, reports, photos, onClose }: { id: string; reports: KsReport[]; photos: KsPhoto[]; onClose: () => void }) {
  ...
  const live = hydrateSoftrReport(row);
  return (
    <PrintChrome docId={live.id} onClose={onClose} kind="ks">
      <KsDoc report={live} photos={photos} />
    </PrintChrome>
  );
}
